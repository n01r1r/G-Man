...
Mesh Model::processMesh(aiMesh *mesh, const aiScene *scene) {
    ...
    for(unsigned int i = 0; i < mesh->mNumVertices; i++) {
        ...
        if (vertex.Position.x > maxBound.x) maxBound.x = vertex.Position.x;
        if (vertex.Position.y > maxBound.y) maxBound.y = vertex.Position.y;
        if (vertex.Position.z > maxBound.z) maxBound.z = vertex.Position.z;
    }
